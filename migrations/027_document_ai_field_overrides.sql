...
CREATE INDEX IF NOT EXISTS idx_document_ai_field_overrides_document
  ON public.document_ai_field_overrides(document_source, document_id, reviewed_at DESC);
...
COMMENT ON COLUMN public.document_ai_field_overrides.idempotency_key IS
  'Deterministic SHA-256 evidence key generated by the server contract to prevent duplicate correction records.';
