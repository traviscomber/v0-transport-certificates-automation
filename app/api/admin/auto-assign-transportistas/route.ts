...
/**
 * Auto-assign transportistas using the canonical CSV seed mapping.
 * This is an administrative operation and requires a verified server-side admin session.
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (const exec of executives) {
      execByFirstName.set(exec.full_name.split(' ')[0], { id: exec.id, full_name: exec.full_name })
    }
    ...
    for (const csvRow of csvData) {
      ...
      if (!executive) {
        errors.push(`Executive ${mappedName} not found in DB`)
        continue
      }
    ...
    console.error('[admin] Error in CSV-based auto-assign:', error)
    return NextResponse.json(
