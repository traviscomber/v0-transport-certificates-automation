...
export async function POST(
  ...
  try {
    ...
    if (isPdf) {
      // Some PDF parsers may detach/mutate the underlying ArrayBuffer. Keep
      // independent byte copies so OCR always receives the original document.
      const sourceBytes = new Uint8Array(buffer)
      const nativeExtractionBytes = new Uint8Array(sourceBytes)
      const ocrBytes = new Uint8Array(sourceBytes)

      try {
        const { text: textArray } = await extractText(nativeExtractionBytes)
      ...
      if (rawDocumentText.trim().length >= 10) {
        ...
        aiExtraction = await extractDocumentFromPdfBuffer(ocrBytes, doc.document_type || 'documento')
