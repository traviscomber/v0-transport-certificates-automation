...
export function TransporterManagement({
  ...
  const filteredTransporters = useMemo(() => {
    ...
    return initialTransporters.filter((transporter) => {
  ...
  }, [searchTerm, initialTransporters])

  const stats = useMemo(() => {
    const active = initialTransporters.filter((t) => t.is_active !== false).length
    const queried = initialTransporters.filter((t) => Boolean(mercadoPublicoByRut[normalizeRut(t.rut)])).length
    const registered = initialTransporters.filter((t) => {
    ...
    return {
      total: initialTransporters.length,
      active,
      inactive: initialTransporters.length - active,
      queried,
      registered,
    }
  }, [initialTransporters, mercadoPublicoByRut])
    ...
    <div className="space-y-6">
      ...
      <Card className="border-slate-700/50 bg-gradient-to-br from-slate-800 to-slate-900">
        <CardHeader>
          ...
          <CardDescription>{filteredTransporters.length} transportistas encontrados</CardDescription>
        </CardHeader>
        <CardContent>
