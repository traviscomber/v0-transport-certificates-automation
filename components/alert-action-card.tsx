...
import { AlertCircle, AlertTriangle, Info, CheckCircle, XCircle, MessageSquare, Loader, Building2 } from 'lucide-react'
...
export function AlertActionCard({ alert, onAction }: AlertActionCardProps) {
  ...
  const subcontractorIdentity = transportistaNombre ? (
    ...
    return (
      <div className={`p-4 border rounded-lg flex items-start gap-4 ${getAlertColor(alert.priority, alert.is_dismissed, alert.status)}`}>
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-foreground/85 dark:text-slate-200 mt-2">{alert.message}</p>
          {alert.action_notes && (
    ...
    <div className={`p-4 border rounded-lg flex flex-col gap-4 transition-all ${getAlertColor(alert.priority, alert.is_dismissed)}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {alert.metadata && (
            <div className="mt-2 text-xs text-slate-400 space-y-1">
              ...
              {!transportistaNombre && alert.metadata.transportista_id && (
                <p>Transportista ID: <span className="text-slate-300 font-mono">{alert.metadata.transportista_id}</span></p>
              )}
      ...
      {!selectedAction && (
        <div className="flex gap-2 flex-wrap">
          <Button size="sm" onClick={() => setSelectedAction('approve')} className="bg-green-600 hover:bg-green-700 text-white text-xs" disabled={isLoading}>
            <CheckCircle className="w-3 h-3 mr-1" /> Aprobar
          </Button>
          <Button size="sm" onClick={() => setSelectedAction('reject')} className="bg-red-600 hover:bg-red-700 text-white text-xs" disabled={isLoading}>
            <XCircle className="w-3 h-3 mr-1" /> Rechazar
          </Button>
          <Button size="sm" onClick={() => setSelectedAction('request_info')} className="bg-yellow-600 hover:bg-yellow-700 text-white text-xs" disabled={isLoading}>
            <MessageSquare className="w-3 h-3 mr-1" /> Solicitar Info
          </Button>
        </div>
      )}
      ...
      {selectedAction && (
        <div className="space-y-2 p-3 bg-slate-900/50 rounded">
          ...
          <div className="flex gap-2 justify-end">
            <Button size="sm" variant="outline" onClick={() => { setSelectedAction(null); setNotes('') }} className="text-xs" disabled={isLoading}>
              Cancelar
            </Button>
            <Button
              size="sm"
              onClick={() => handleAction(selectedAction)}
              className={`text-xs text-white ${selectedAction === 'approve' ? 'bg-green-600 hover:bg-green-700' : selectedAction === 'reject' ? 'bg-red-600 hover:bg-red-700' : 'bg-yellow-600 hover:bg-yellow-700'}`}
              disabled={isLoading}
            >
              {isLoading ? (
                <><Loader className="w-3 h-3 mr-1 animate-spin" />Procesando...</>
              ) : (
                <>{selectedAction === 'approve' && 'Confirmar Aprobación'}{selectedAction === 'reject' && 'Confirmar Rechazo'}{selectedAction === 'request_info' && 'Solicitar'}</>
              )}
            </Button>
          </div>
        </div>
