...
export const CHILEAN_DOCUMENTS: Record<string, ChileanDocumentReference> = {
  ...
  licencia_a4: {
    ...
    mandatory_validations: [
      'RUT coincide con otros documentos',
      'No está vencida',
      'Categoría exacta es A4',
      'Firma presente',
      'Foto clara'
    ],
  ...
  seguro_responsabilidad: {
    ...
    mandatory_validations: [
      'Cobertura mínima $25.000.000',
      'Patente coincide',
      'No está vencido',
      'Aseguradora es válida'
    ],
  ...
  certificado_capacitacion_123: {
    ...
    mandatory_validations: [
      'RUT coincide',
      'No está vencido',
      'Instituto es acreditada',
      'Tipo capacitación válido'
    ],
  ...
  rut_empresa: {
    ...
    mandatory_validations: [
      'RUT empresa correcto',
      'Estado VIGENTE',
      'No está vencido',
      'Giro relacionado a transporte'
    ],
  ...
  inscripcion_mtt: {
    ...
    mandatory_validations: [
      'Número inscripción válido',
      'RUT coincide',
      'No vencida',
      'Tipo servicio coincide'
    ],
...
export function getDocumentRequirements(documentType: string) {
  return CHILEAN_DOCUMENTS[documentType] || null
}
...
export function getDocumentsByCategory(category: 'conductor' | 'vehiculo' | 'empresa' | 'especial') {
  return Object.values(CHILEAN_DOCUMENTS).filter(doc => doc.category === category)
}

// Función para obtener documentos críticos
export function getCriticalDocuments() {
  return Object.values(CHILEAN_DOCUMENTS).filter(doc => doc.priority === 'CRÍTICA')
}
