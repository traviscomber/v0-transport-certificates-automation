...
/**
 * Authenticate the reprocess boundary before any privileged database access.
 *
 * Human requests use the canonical application authentication flow. The F30
 * backfill is the only internal caller and must present CRON_SECRET explicitly;
 * a caller cannot claim `source=f30_backfill` and fall back to user auth.
 */
...
/**
 * Authorize access only after the document table has been resolved.
 *
 * Cron execution is intentionally restricted to the F30/subcontractor surface.
 * Human requests reuse the same assignment-aware authorization contract as the
 * canonical document status mutation endpoint.
 */
